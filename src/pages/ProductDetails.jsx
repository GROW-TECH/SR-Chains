...
export default function ProductDetails() {
  ...
  const getWishlist = () =>
    JSON.parse(sessionStorage.getItem(WISHLIST_KEY)) || [];

  const isInWishlist = (pid) => getWishlist().some((p) => p.id === pid);
  ...
  /* ================= ADD TO CART ================= */
  const addToCart = () => {
    const hasQty = Object.values(sizeQtyMap).some((q) => q > 0);
    if (!hasQty) {
    ...
    Object.entries(sizeQtyMap).forEach(([size, qty]) => {
      ...
      const purePayable = (weight * (purity + product.wastage)) / 100;
      const makingCharges = (product.makingCharge / 1000) * weight;
      ...
      const existing = cart.find(
        (i) => i.product.id === product.id && i.size === sizeNum
      );
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 text-sm">
        ...
        <div className="grid lg:grid-cols-2 gap-10 mb-12">
          ...
          <div>
            ...
            >
              ...
              <button
                onClick={prevImage}
                className="absolute left-3 top-1/2 bg-white w-9 h-9 rounded-full shadow"
              >
                ‹
              </button>
              <button
                onClick={nextImage}
                className="absolute right-3 top-1/2 bg-white w-9 h-9 rounded-full shadow"
              >
                ›
              </button>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border rounded-xl p-4 space-y-2">
              <Detail
                label="Category"
                value={product.categorySlug.replace(/-/g, " ")}
              />
              <Detail label="Size Range" value={product.sizeRange} />
              <Detail
                label="Weight / Inch"
                value={`${product.weightPerInch} g`}
              />
              <Detail label="Wastage" value={`${product.wastage}%`} />
              <Detail
                label="Making Charges"
                value={`₹${product.makingCharge} / kg`}
              />
            </div>
        ...
        <div className="border rounded-xl p-6 mb-10">
          ...
          {[
            ...
            <div key={r.id} className="border-b pb-4 mb-4 last:border-0">
